...
//
//            // In heightmap format
//            heightMapPoints.push_back(Point_3(i, (heightsBand->GetYSize() - 1) - j, height));
//
//            // In Latitude, Longitude, Height format
//            float lat = tileBounds.getMinY() + ((tileBounds.getMaxY() - tileBounds.getMinY()) * ((((float)heightsBand->GetYSize() - 1) - j)/(float)heightsBand->GetYSize()));
//            float lon = tileBounds.getMinX() + ((tileBounds.getMaxX() - tileBounds.getMinX()) * ((float)i/(float)heightsBand->GetXSize()));
//
////            std::cout << "heightsBand->GetXSize() = " << heightsBand->GetXSize() << std::endl ;
////            std::cout << "i = " << i << std::endl ;
////            std::cout << "(i/heightsBand->GetXSize()) = " << i/heightsBand->GetXSize() << std::endl ;
...
//    std::cout << "heightMapPoints.size() = " << heightMapPoints.size() << std::endl ;
//    if ( tileWestVertices.size() > 0 ) {
...
//    int k = 0 ;
    for ( int i = startX; i < tileSize; i++ ) {
        for (int j = startY; j < tileSize; j++) {
            float height = rasterHeights[j * tileSize + i];
//            float height = rasterHeights[i * tileSize + j];
//            float height = rasterHeights[k];
//            k++ ;

//            // WARNING: This is just for Canada data, should not be included in the final code...
//            if ( height < -1000 )
//                height = 0 ;
...
//            std::cout << "lat = " << lat << std::endl ;
//            std::cout << "lon = " << lon << std::endl ;
//            std::cout << "max lat = " << tileBounds.getMaxY() << std::endl ;
//            std::cout << "max lon = " << tileBounds.getMaxX() << std::endl ;
    ...
    std::cout << "min lat = " << tileBounds.getMinY() << std::endl ;
    std::cout << "expected min lat = " << minLat << std::endl ;
    std::cout << "min lon = " << tileBounds.getMinX() << std::endl ;
    std::cout << "expected min lon = " << minLon << std::endl ;
    std::cout << "max lat = " << tileBounds.getMaxY() << std::endl ;
    std::cout << "expected max lat = " << maxLat << std::endl ;
    std::cout << "max lon = " << tileBounds.getMaxX() << std::endl ;
    std::cout << "expected max lon = " << maxLon << std::endl ;
    ...
    std::vector< Point_3 > ecefPoints ;
    ecefPoints.reserve(latLonPoints.size()) ;
    double minEcefX = 999999, maxEcefX = -999999, minEcefY = 999999, maxEcefY = -999999, minEcefZ = 999999, maxEcefZ = -999999 ;
    for ( int i = 0; i < latLonPoints.size(); i++ ) {
        GeographicLib::Geocentric earth(GeographicLib::Constants::WGS84_a(), GeographicLib::Constants::WGS84_f()) ;
        double tmpx, tmpy, tmpz ;
    ...
    // Get the middle point as the centroid
//    Point_3 centroid = CGAL::centroid( ecefPoints.begin(), ecefPoints.end(), CGAL::Dimension_tag<0>() ) ;
//    header.CenterX = centroid.x() ;
//    header.CenterY = centroid.y() ;
//    header.CenterZ = centroid.z() ;

    // Get the middle point as the true center (lat/lon -> ecef)
...
//
//    std::cout << "midLat = " << midLat << std::endl ;
//    std::cout << "midLon = " << midLon << std::endl ;
//    std::cout << "midH = " << midH << std::endl ;
//    std::cout << "header.CenterX = " << header.CenterX << std::endl ;
//    std::cout << "header.CenterY = " << header.CenterY << std::endl ;
//    std::cout << "header.CenterZ = " << header.CenterZ << std::endl ;

    // Get the middle point as the true center (from ecef directly)
    ...
    for ( std::vector<Point_3>::iterator it = heightMapPoints.begin(); it != heightMapPoints.end(); ++it ) {
        ...
        if (u > 32767)
            std::cout << "WRONG U!" << u << std::endl ;
        if (v > 32767)
            std::cout << "WRONG V!" << v << std::endl ;
        if (h > 32767) {
            std::cout << "WRONG H!" << h << std::endl;
            std::cout << "height = " << CGAL::to_double(it->z()) << std::endl;
        }

//        std::cout << "h = " << h << std::endl;
//        std::cout << "height = " << CGAL::to_double(it->z()) << std::endl;
        if (h == 32767)
            std::cout << "h max included" << std::endl;
//        if (h == 0)
//            std::cout << "h min included" << std::endl;
...
//        vertexData.height.push_back(h) ;
        ...
        if (u > 32767)
            std::cout << "WRONG U2!" << u << std::endl ;
        if (v > 32767)
            std::cout << "WRONG V2!" << v << std::endl ;
        if (h > 32767) {
            std::cout << "WRONG H2!" << h << std::endl;
            std::cout << "height = " << CGAL::to_double(it->z()) << std::endl;
        }

        if (h == 32767)
            std::cout << "h2 max included" << std::endl;
//        if (h == 0)
//            std::cout << "h2 min included" << std::endl;
...
//    std::cout << "Unsorted indices" << std::endl ;
//    for (int i=0; i < 50; i=i+3) {
//        std::cout << indexData.indices[i] << ", " << indexData.indices[i+1] << ", " << indexData.indices[i+2] << std::endl ;
//    }

    // Optimize the resulting mesh
